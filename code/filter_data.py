...
def filter_tcrb_data(rtcr_ref: dict, tcrb_data_filenames, threshold, max_insert_size, flt_strain, delim, out_filename):
    ...
    :param max_insert_size: int - The limit insert size from which sequences are not included in the filtered data.
    :param flt_strain: str -
    ...
    for file in tcrb_data_filenames:
        with open(file, 'r') as csvfile:
            ...
            for row in reader:
                ...
                if int(reads) > threshold and int(line[6]) < max_insert_size and strain == flt_strain:
